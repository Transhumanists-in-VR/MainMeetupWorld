...
void applyDecals(inout float4 albedo, inout float3 decalEmission)
{
    ...
    decalScale = float4(1,1,0,0);
        ...
        decalColor = POI2D_SAMPLER_PAN(_DecalTexture, _MainTex, uv, float4(0.2,2,0,0)) * float4(0.5209957,0.2622507,0.05780542,1);
    #else
        uv = decalUV(float(0), float4(0.5,0.5,0,0), float(0) + decalRotation, float(0), decalScale, float(0));
        decalColor = float4(0.5209957,0.2622507,0.05780542,1);
    #endif
    ...
    #endif
    decalEmission += decalColor.rgb * decalColor.a * max(float(3.3) + audioLinkDecalEmission0, 0);
    #ifdef GEOM_TYPE_BRANCH_DETAIL
        decalScale = float4(1,1,0,0);
        decalRotation = 0;
        #if defined(PROP_DECALTEXTURE1) || !defined(OPTIMIZER_ENABLED)
            ...
            #endif
            uv = decalUV(float(0), float4(0.5,0.5,0,0), float(0) + decalRotation, float(0), decalScale, float(0.01));
            decalColor = POI2D_SAMPLER_PAN(_DecalTexture1, _MainTex, uv, float4(0,0.3,0,0)) * float4(0.5209957,0.2622507,0.05780542,1);
        #else
            uv = decalUV(float(0), float4(0.5,0.5,0,0), float(0) + decalRotation, float(0), decalScale, float(0.01));
            decalColor = float4(0.5209957,0.2622507,0.05780542,1);
        #endif
        ...
        #endif
        decalEmission += decalColor.rgb * decalColor.a * max(float(3.1) + audioLinkDecalEmission1, 0);
    #endif
